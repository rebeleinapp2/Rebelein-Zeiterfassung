...
const GlassDatePicker: React.FC<GlassDatePickerProps> = ({ value, onChange, onClose, gracePeriodCutoff, rangeMode, rangeStart, rangeEnd, onRangeChange }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-xs relative shadow-2xl !p-0 overflow-hidden ring-1 ring-white/20">
        {/* Header */}
        <div className={`p-4 border-b border-white/10 flex items-center justify-between ${rangeMode ? 'bg-gradient-to-r from-blue-900/50 to-indigo-900/50' : 'bg-gradient-to-r from-teal-900/50 to-emerald-900/50'}`}>
        ...
        <div className="flex border-t border-white/10">
          {rangeMode && rangeSelectionStep === 'end' && (
            <button
              ...
              className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-sm text-blue-300/60 hover:text-blue-300 uppercase tracking-wider font-bold transition-colors"
            >
              Zurücksetzen
            </button>
          )}
          <button
            onClick={onClose}
            className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-sm text-white/50 uppercase tracking-wider font-bold transition-colors"
          >
            Abbrechen
          </button>
        </div>
