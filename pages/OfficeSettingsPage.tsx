...
const OfficeSettingsPage: React.FC = () => {
    ...
    const renderHolidays = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div>
                    <h3 className="text-xl font-bold text-foreground mb-1">Feiertags-Konfiguration</h3>
                    <div className="flex items-center gap-2">
                        <p className="text-muted-foreground text-sm">Diese Einstellungen gelten global für alle Mitarbeiter.</p>
                        <div className="flex items-center bg-muted border border-border rounded-lg px-2 py-0.5 gap-2 ml-2">
                            <button onClick={() => setViewYear(y => y - 1)} className="text-teal-400 hover:text-foreground transition-colors"><ChevronLeft size={14} /></button>
                            <span className="text-xs font-bold text-foreground w-10 text-center">{viewYear}</span>
                            <button onClick={() => setViewYear(y => y + 1)} className="text-teal-400 hover:text-foreground transition-colors"><ChevronRight size={14} /></button>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-muted rounded-2xl border border-border overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-muted">
                    {getBavarianHolidays(viewYear).map((h) => {
                        ...
                        return (
                            <div key={h.id} className={`bg-card p-4 flex items-center justify-between transition-colors ${isActive ? 'bg-muted' : 'opacity-40'}`}>
                                <div className="flex items-center gap-3">
                                    <div onClick={() => setHolidayConfig(prev => ({ ...prev, [h.id]: !isActive }))} className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all cursor-pointer ${isActive ? 'bg-rose-500/20 border-rose-500/40 text-rose-300' : 'bg-muted border-border text-muted-foreground'}`}>
                                        <PartyPopper size={20} />
                                    </div>
                                    <div>
                                        <div className={`font-bold transition-colors ${isActive ? 'text-foreground' : 'text-muted-foreground'}`}>{h.name}</div>
                                        <div className="flex items-center gap-2 mt-1">
                                            <input type="date" value={displayDate.toISOString().split('T')[0]} onChange={(e) => setHolidayOverrides(prev => ({ ...prev, [viewYear]: { ...(prev[viewYear] || {}), [h.id]: e.target.value } }))} className={`bg-input border rounded px-2 py-0.5 text-[10px] font-mono font-bold outline-none ${isOverridden ? 'border-amber-500/50 text-amber-400' : 'border-border text-teal-400/70'}`} />
                                ...
                                <div className={`w-6 h-6 rounded-lg border flex items-center justify-center ${isActive ? 'bg-emerald-500 border-emerald-400 text-foreground' : 'bg-muted border-border text-transparent'}`}><Check size={14} /></div>
                            </div>
                        );
                    })}
                </div>
    ...
    const renderMonthClosing = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
                <h3 className="text-xl font-bold text-foreground mb-1">Monatsabschluss & Sperrungen</h3>
                <p className="text-muted-foreground text-sm">Gesperrte Monate können von Mitarbeitern nicht mehr bearbeitet werden.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[...Array(12)].map((_, i) => {
                    ...
                    return (
                        <div key={monthStr} className="flex items-center justify-between bg-muted border border-border p-4 rounded-2xl hover:bg-card transition-colors">
                            <div>
                                <div className="font-bold text-foreground">{d.toLocaleDateString('de-DE', { month: 'long', year: 'numeric' })}</div>
    ...
    const renderDepartments = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
                <h3 className="text-xl font-bold text-foreground mb-1">Abteilungs-Verwaltung</h3>
                <p className="text-muted-foreground text-sm">Zuständigkeiten und Vertretungsregelungen für die Freigabe.</p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {departments.map(dept => (
                    <GlassCard key={dept.id} className="!p-0 border-border overflow-hidden">
                        <div className="bg-muted px-4 py-3 border-b border-border flex justify-between items-center">
                            ...
                            <div className="flex gap-1.5">
                                <div className={`w-2 h-2 rounded-full ${dept.is_substitute_active ? 'bg-teal-500 shadow-[0_0_8px_rgba(20,184,166,0.6)]' : 'bg-card'}`} />
                                <div className={`w-2 h-2 rounded-full ${dept.is_retro_substitute_active ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.6)]' : 'bg-card'}`} />
                            </div>
                        </div>
                        <div className="p-4 space-y-4">
                            <div>
                                <label className="text-[10px] text-muted-foreground uppercase font-bold block mb-1.5">Verantwortlicher</label>
                                <select className="w-full bg-input text-foreground text-sm rounded-xl border border-border px-3 py-2 outline-none" value={dept.responsible_user_id || ''} onChange={(e) => updateDepartment(dept.id, { responsible_user_id: e.target.value })}>
                            ...
                            <div className="flex items-center justify-between p-3 bg-muted rounded-xl border border-border">
                                <span className="text-xs text-muted-foreground font-bold">Vertretung aktiv</span>
                                <button onClick={() => updateDepartment(dept.id, { is_substitute_active: !dept.is_substitute_active })} className={`w-10 h-6 rounded-full p-1 transition-all flex items-center ${dept.is_substitute_active ? 'bg-teal-500 justify-end' : 'bg-card justify-start'}`}><div className="w-4 h-4 rounded-full bg-white shadow-sm" /></button>
                            </div>

                            {/* Additional Responsible Users */}
                            <div className="pt-2">
                                <label className="text-[10px] text-muted-foreground uppercase font-bold block mb-1.5">Weitere Zuständige</label>
                                <div className="bg-input rounded-xl p-2 border border-border space-y-2">
                                        ...
                                        {(dept.additional_responsible_ids || []).map(id => {
                                            ...
                                            return (
                                                <div key={id} className="flex items-center gap-1 bg-teal-500/10 text-teal-200 pl-2 pr-1 py-1 rounded-lg text-[10px] border border-teal-500/20">
                                                    ...
                                                    }} className="hover:bg-teal-500/20 rounded p-0.5 text-teal-400 hover:text-foreground">
                                                        <X size={10} />
                                                    </button>
                                                </div>
                                            );
                                        })}
                                        {(dept.additional_responsible_ids || []).length === 0 && <span className="text-muted-foreground text-[10px] italic px-1">Keine</span>}
                                    </div>
                                    <select 
                                        className="w-full bg-transparent text-teal-100/50 text-[10px] outline-none cursor-pointer border-t border-border pt-1.5 mt-1"
                                        value=""
                                        onChange={(e) => {
                            ...
                            <div className="pt-4 border-t border-border space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <button onClick={() => updateDepartment(dept.id, { is_retro_substitute_active: !dept.is_retro_substitute_active })} className={`w-10 h-6 rounded-full p-1 transition-all flex items-center ${dept.is_retro_substitute_active ? 'bg-orange-500 justify-end' : 'bg-card justify-start'}`}><div className="w-4 h-4 rounded-full bg-white shadow-sm" /></button>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <label className="text-[9px] text-muted-foreground uppercase font-bold block mb-1 ml-1">Chef</label>
                                        <select className="w-full bg-input text-orange-100 text-[11px] rounded-lg border border-orange-500/10 p-1.5 outline-none" value={dept.retro_responsible_user_id || ''} onChange={(e) => updateDepartment(dept.id, { retro_responsible_user_id: e.target.value })}>
                                    ...
                                    <div>
                                        ...
                                        <select className="w-full bg-input text-orange-100 text-[11px] rounded-lg border border-orange-500/10 p-1.5 outline-none" value={dept.retro_substitute_user_id || ''} onChange={(e) => updateDepartment(dept.id, { retro_substitute_user_id: e.target.value })}>
                                            <option value="">Standard</option>
                                            {users.map(u => <option key={u.user_id} value={u.user_id} className="bg-slate-900">{u.display_name}</option>)}
                                        </select>
                                    </div>
    ...
    const renderExport = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
                <h3 className="text-xl font-bold text-foreground mb-1">Stunden-Export (Batch)</h3>
                <p className="text-muted-foreground text-sm">Laden Sie Stundenberichte für mehrere Mitarbeiter gleichzeitig herunter.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between bg-muted p-2 rounded-2xl border border-border">
                            <button onClick={() => setSelectedExportMonth(new Date(selectedExportMonth.getFullYear(), selectedExportMonth.getMonth() - 1, 1))} className="p-2 hover:bg-card rounded-xl text-foreground"><ChevronLeft /></button>
                            <span className="font-bold text-foreground">{selectedExportMonth.toLocaleDateString('de-DE', { month: 'long', year: 'numeric' })}</span>
                            <button onClick={() => setSelectedExportMonth(new Date(selectedExportMonth.getFullYear(), selectedExportMonth.getMonth() + 1, 1))} className="p-2 hover:bg-card rounded-xl text-foreground"><ChevronRight /></button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            {['projects', 'attendance', 'monthly_report'].map(type => (
                                <button key={type} onClick={() => setExportTypes(prev => ({ ...prev, [type]: !prev[type as keyof typeof exportTypes] }))} className={`w-full p-3 rounded-xl border flex items-center gap-3 transition-all ${exportTypes[type as keyof typeof exportTypes] ? 'bg-teal-500/10 border-teal-500/30 text-foreground' : 'bg-muted border-border text-muted-foreground'}`}>
                ...
                <div className="flex flex-col h-full space-y-2">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => setSelectedExportUsers(selectedExportUsers.length === users.length ? [] : users.map(u => u.user_id!))} className="text-[10px] font-bold text-muted-foreground hover:text-foreground uppercase">Alle wählen</button>
                    </div>
                    <div className="bg-muted rounded-2xl border border-border overflow-y-auto max-h-[400px] p-2 custom-scrollbar">
                        {users.map(u => (
                            <button key={u.user_id} onClick={() => setSelectedExportUsers(prev => prev.includes(u.user_id!) ? prev.filter(id => id !== u.user_id) : [...prev, u.user_id!])} className={`w-full p-3 rounded-xl mb-1 flex items-center gap-3 text-left transition-all ${selectedExportUsers.includes(u.user_id!) ? 'bg-card text-foreground' : 'text-muted-foreground hover:bg-muted'}`}>
    ...
    const renderSchoolHolidays = () => {
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                    <div>
                        <h3 className="text-xl font-bold text-foreground mb-1">Schulferien (Bayern)</h3>
                        <div className="flex items-center gap-2">
                            <p className="text-muted-foreground text-sm">Zeiträume werden in den Kalendern farblich markiert.</p>
                            <div className="flex items-center bg-muted border border-border rounded-lg px-2 py-0.5 gap-2 ml-2">
                                <button onClick={() => setViewYear(y => y - 1)} className="text-teal-400 hover:text-foreground transition-colors"><ChevronLeft size={14} /></button>
                                <span className="text-xs font-bold text-foreground w-10 text-center">{viewYear}</span>
                                <button onClick={() => setViewYear(y => y + 1)} className="text-teal-400 hover:text-foreground transition-colors"><ChevronRight size={14} /></button>
                    ...
                    <div className="flex flex-wrap gap-3 w-full md:w-auto shrink-0 items-center">
                        ...
                        <div className="flex items-center gap-2 bg-muted border border-border rounded-xl px-3 py-2">
                            <span className="text-xs font-bold text-muted-foreground">Kalender</span>
                            <button
                                type="button"
                                onClick={toggleSchoolHolidayVisibility}
                                className={`w-10 h-5 rounded-full p-0.5 transition-colors duration-200 relative ${schoolHolidayConfig.showInCalendar ? 'bg-teal-500' : 'bg-card'}`}
                            >
                ...
                <div className="bg-muted rounded-2xl border border-border overflow-hidden">
                    <div className="divide-y divide-white/5">
                        {SCHOOL_HOLIDAY_TEMPLATES.map((template) => {
                            ...
                            return (
                                <div key={template.id} className={`p-4 flex flex-col md:flex-row md:items-center gap-3 transition-colors ${hasData ? 'bg-muted' : ''}`}>
                                    <div className="flex items-center gap-3 md:w-48 shrink-0">
                                        <span className="text-xl">{template.icon}</span>
                                        <span className={`font-bold transition-colors ${hasData ? 'text-foreground' : 'text-muted-foreground'}`}>{template.name}</span>
                                    </div>
                                    <div className="flex items-center gap-2 flex-1">
                                        <input
                                            ...
                                            className="flex-1 bg-input border border-border rounded-lg px-3 py-2 text-sm text-teal-400 font-mono outline-none focus:border-teal-500/50 transition-colors"
                                        />
                                    ...
                                    {hasData && (
                                        <div className="text-[10px] text-muted-foreground font-mono md:w-20 text-right">
    ...
    const renderContent = () => {
        switch (activeCategory) {
            ...
            default: return <div className="flex flex-col items-center justify-center h-64 text-muted-foreground italic"><Database size={48} className="mb-4 opacity-10" /> Sektion im Aufbau</div>;
        }
    };
        ...
        <div className="p-6 pb-24 h-full overflow-hidden flex flex-col md:max-w-7xl md:mx-auto w-full">
            <div className="flex items-center justify-between mb-8 shrink-0">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/office')} className="p-2 bg-muted hover:bg-card rounded-xl text-muted-foreground hover:text-foreground transition-colors border border-border"><ChevronLeft size={24} /></button>
                    <div><h1 className="text-3xl font-bold text-foreground tracking-tight">Verwaltung</h1><p className="text-muted-foreground text-sm">Systemweite Einstellungen & Konfiguration</p></div>
                </div>
            </div>
            <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0">
                <div className="w-full md:w-64 shrink-0 flex flex-col gap-2 overflow-y-auto pr-2 custom-scrollbar">
                    {CATEGORIES.map(cat => (
                        <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 text-sm font-bold border ${activeCategory === cat.id ? 'bg-gradient-to-r from-teal-500 to-emerald-600 text-foreground border-teal-400/50 shadow-lg shadow-teal-900/20' : 'bg-muted text-muted-foreground border-border hover:bg-card hover:text-muted-foreground'}`}>
                            <cat.icon size={20} className={activeCategory === cat.id ? 'text-foreground' : 'text-muted-foreground'} /> {cat.name}
                        </button>
                    ))}
                </div>
                <div className="flex-1 min-w-0 overflow-y-auto pr-2 custom-scrollbar">
                    <GlassCard className="h-full !p-8 border-border bg-input backdrop-blur-xl relative !overflow-visible">
                        <div className="absolute top-0 right-0 -mr-20 -mt-20 w-64 h-64 bg-teal-500/5 rounded-full blur-3xl pointer-events-none" />
                        {loading ? <div className="flex flex-col items-center justify-center h-full gap-4 text-muted-foreground"><div className="w-10 h-10 border-2 border-teal-500 border-t-transparent rounded-full animate-spin"></div><span className="font-bold text-sm tracking-widest uppercase">Lade Konfiguration...</span></div> : renderContent()}
                    </GlassCard>
                </div>
            </div>
        </div>
